# Step 0: VS Code + Claude Code の環境をつくる
...
3. PCの制約（管理者権限がない、社内プロキシで弾かれる等）でどうしても解決しない場合は、この環境構築を一旦保留し、**ブラウザ完結の代替環境**（Claude Code on the web）に切り替えてStep 1以降を先に進める。インストールは後回しにしてよい。
...
こちらは画面操作がない分、コマンドの意味を理解していないと詰まりやすいので、Step 0では推奨しません。慣れてきたら試してみてください。
</details>
