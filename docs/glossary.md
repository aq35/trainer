...
**不具合を理論上100%防ぐ方法は1つだけあります。全部の組み合わせを試すことです。——できません。**
...
<img class="ui-mock" src="media/cannot-test-all.svg" alt="全部の組み合わせは試せないので必ずやらないことを決めている。分けるのは、やらなかったことを言えるようにするため">

<div class="checkpoint">
<strong>だから、どんな現場も必ず「やらないこと」を決めています。</strong><br>
テストは、<b>やることを決める作業ではなく、やらないことを決める作業</b>です。<br>
1つの大きなテストにすると<b>何をやらなかったかが言えない</b>ので、切り口ごとに分けます。<b>これが4段に分かれている理由です。</b>
</div>

ここから2つ、注意が出てきます。

- **項目表を作ること自体が目的になると、本末転倒です。** 項目は納品物ではなく、**範囲を言うための道具**です
- **「やらない」の線は動かせます。** 自動化とAIで安くできた分だけ、やる側に移せます
...
**`assert` は「コードの動き」と「テストを書いた人が思っている仕様」を比べています。**

つまり単体テストが緑になっても、保証されるのは **コードが書いた人の想定どおりに動くこと** までです。**その想定が仕様と違っていれば、緑のまま仕様を満たしません。**

だから単体テストの上に、仕様と突き合わせる**機能テスト**が別に要ります。

<div class="checkpoint">
<strong>仕様書からAIにコードとテストを作らせた場合は、どうなるか。</strong><br>
<b>緑の意味は、むしろ弱くなります。</b>仕様のあいまいな所を、AIがコードとテストの<b>両方で同じように解釈する</b>からです。<br>
片方の間違いをもう片方が見つける形になっていません。<b>生成元が同じものは、互いの検算になりません。</b>
</div>

対処は2つです。

- **あいまいな所を先に潰してから作らせる** —— [マトリクス](test.html ':ignore target=_blank')で「決まっていない欄」を洗い出す
- **テストの期待値の行だけは、自分で読んで決める** —— ここは人が残す場所です
